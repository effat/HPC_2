...
Curveopt<-function(norm_rank, x_cord_seq, my_pop, conf_pop, instancedf){
  ...
  my_target.runner<-function(migration_vect, pop = my_pop, confPop = conf_pop){
    ...
    maxgen_val <-50
    best_cost  <-0
    best_cost_array<-rep(0, maxgen_val)
    ...
    for(i in 1:maxgen_val){
      ...
      ##get the best member
     # store best co9st of each gen
      
      best_cost_array[i]<-costs[1]
    ...
    ##best cost is the mean of best_cost_array
